 ...
 * @LastEditors: 张泽雨
 * @LastEditTime: 2024-04-03 12:14:55
 * @FilePath: \vue3ts\src\layout\index.vue
-->
<template>
  <van-sticky ref="stickyRef">
    <van-nav-bar :title="isString($route.meta?.title) ? $route.meta?.title : ''">
      <template #left>
  ...
  <van-tabbar ref="tabbarRef" route>
    <template v-for="item in main" :key="item.name">
      <van-tabbar-item :to="item.path" :icon="isString(item.meta?.icon) ? item.meta?.icon : ''">
        {{ item.meta?.title }}
      </van-tabbar-item>
    </template>
  </van-tabbar>
